...
const DISTANCE_GOALS = [
  ...
  { label: '15 kilometers', value: 15000 },
  { label: '25 kilometers', value: 25000 },
  { label: '50 kilometers', value: 50000 },
]
...
const Cycling = () => {
  ...
  const [isTimedRide, setIsTimedRide] = useState(false)
  const [activeGoalLabel, setActiveGoalLabel] = useState('No ride in progress')
  ...
  const startRide = async ({ distanceGoal = 0, timeGoal = null, label }) => {
    stopActiveRide()

    const { status } = await Location.requestForegroundPermissionsAsync()
    if (status !== 'granted') {
      alert('Location permission is required to track a ride.')
      return
    }

    setActiveGoalLabel(label)
    setIsTimedRide(Boolean(timeGoal))
    ...
    logWorkoutActivity('cardio', 'Cycling', label).catch((error) => {
    ...
    if (timeGoal) {
      timerIdRef.current = setInterval(() => {
        ...
        if (elapsed >= timeGoal) endRide()
      }, 500)
    }
  }

  useEffect(() => {
    return () => stopActiveRide()
  }, [])

  return (
    <AppShell
      title="Cycling"
      subtitle="Track a distance-based or time-based ride with live speed and distance updates using your device location."
    >
      <SectionCard style={styles.statusCard}>
        ...
        <Pressable style={styles.stopButton} onPress={stopActiveRide}>
        ...
        <View style={styles.goalGrid}>
          {DISTANCE_GOALS.map((goal) => (
            <Pressable
              ...
              onPress={() => startRide({ distanceGoal: goal.value, label: goal.label })}
            >
        ...
        <View style={styles.goalGrid}>
          {TIME_GOALS.map((goal) => (
            <Pressable
              key={goal.label}
              style={styles.goalButton}
              onPress={() => startRide({ timeGoal: goal.value, label: goal.label })}
            >
              <Text style={styles.goalButtonText}>{goal.label}</Text>
            </Pressable>
          ))}
